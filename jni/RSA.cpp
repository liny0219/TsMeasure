...
vector<string> RSA::Split(string src, string pattern) {
...
string RSA::ConvertKeyToString(int src[],int length)
{
	string dec = "";
	for (int i = 0; i < length; i++)
	{
		int &temp = src[i];
		dec += IntToString(temp) += ",";
	}
	return dec;
}

int* RSA::ConvertKeyToIntArray(string src,int length)
{
	int*  dec = new int[length];
	vector < string > list = Split(src, ",");
	for (int i = 0; i < list.size(); i++)
	{
		dec[i] = atoi(list[i].c_str());
	}
	return dec;
}
